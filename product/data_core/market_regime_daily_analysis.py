...
market_meaning 是静态理论机制，不是免责声明段落：只写一般驱动、通常传导和反例，并且必须明确包含“反例”“但”“然而”“未必”“不一定”或“取决于”中的至少一个反例标记。market_meaning 使用中文资产角色，不要复述交易代码、交易所后缀或 canonical symbol（例如 000001.SH、Treasury:2 Yr）。文字中不要出现“当前、最新、收盘、实时、今天、本期、本报告、正在、已经”等事实时态词，也不要写“不是当前价格……”之类的免责声明；不要把任何当前行情数字放进该字段。
