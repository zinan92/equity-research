...
RENDERER_VERSION = "market-regime-weekly-report-renderer-v6"
...
def render_weekly_html(report: Mapping[str, Any]) -> str:
    ...
    for _, chapter, keys in CHAPTERS:
        ...
        for key in keys:
            ...
            pane_parts.append(
                f'<section class="asset-pane" data-pane="{_escape(key)}" data-timeframes="{len(rows)}"><header><h2>{_escape(card["display_name"])}</h2><small>{_escape(str(card["analysis_status"]))}</small></header>{"".join(rows)}<div class="synthesis"><b>多周期结论</b><p>{_escape(summary)}</p></div></section>'
            )
