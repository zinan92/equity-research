...
class DecisionInput:
 ticker:str; context_manifest_hash:str; dossier_id:str; current_price:float; target_price:float|None; quality_score:float|None; risk_score:float|None; liquidity_score:float|None; coverage_passed:bool; sector_exposure:float; current_position:float; cash_weight:float
 def validate(self):
  ...
  if self.current_price<=0 or not all(0<=x<=1 for x in (self.sector_exposure,self.current_position,self.cash_weight)): raise ValueError('decision input range is invalid')
