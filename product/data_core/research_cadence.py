...
def build_cadence_plan(*, now: datetime, last_good: Mapping[str, datetime | None]) -> dict[str, object]:
    ...
    for lane in LANES:
        ...
        rows.append({"name": lane.name, "dependencies": list(lane.dependencies), "refresh_after_seconds": int(lane.refresh_after.total_seconds()), "stale_after_seconds": int(lane.stale_after.total_seconds()), "last_good_at": completed_at.isoformat() if completed_at else None, "state": state})
