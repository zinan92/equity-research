...
另外必须输出 theoretical_implication：这是面向读者的“市场含义与机制解释”，可以使用本周、当前、最新等正常研究语境，把该资产的 K 线观察翻译成市场语言。只能使用请求中的 mechanism 目录作为机制依据，至少引用一个 mechanism_id；不要引用外部事实，也不要输出任何内部 OPS 字段或 provider、fallback、schema、validation 等运行信息。直接回答“这对市场意味着什么”，不要写给程序看的免责声明。

赔率由代码在模型响应通过后计算。不要输出 odds 字段，不要自行填写触发、入场、止损、目标或 R 值；模型不得发明任何价格水平。
