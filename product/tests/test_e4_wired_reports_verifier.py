...
class WiredReportVerifierTest(unittest.TestCase):
    def _files(self, root: Path) -> tuple[Path, Path, Path]:
        ...
        report = {
            ...
            "input_hashes": {"m3": digest(judgment)},
        ...
        paths = root / "receipt.json", root / "queue.json", root / "judgment.json"
        for path, value in zip(paths, (report, queue, judgment)):
            path.write_text(json.dumps(value), encoding="utf-8")
        return paths

    def test_real_receipt_and_complete_pending_queue_pass(self):
        with TemporaryDirectory() as directory:
            receipt, queue, judgment = self._files(Path(directory))
            self.assertEqual(
                MODULE.summary(receipt, queue, judgment)["unreviewed_judgment_count"],
                10,
            )
    ...
    def test_fabricated_queue_body_fails_closed(self):
        with TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "does not match"):
                MODULE.summary(receipt, queue, judgment)
