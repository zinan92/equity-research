...
class ContextPackModelsTest(unittest.TestCase):
    ...
    def _inputs(self, root: Path) -> tuple[Path, Path, Path]:
        ...
        matrix = {"schema_version": "e4-s4-sell-side-matrix-v1", "data_kind": "real", "as_of": "2026-07-25", "research_cutoff": AS_OF, "truth_boundary": {"counts_as_tier_a_or_b": False}, "matrices": [{"ticker": "300750.SZ", "status": "compiled", "matrix": {"matrix_id": "matrix-1", "as_of": "2026-07-25", "research_cutoff": AS_OF, "input_hash": "e" * 64, "rows": [{"report_id": "report-1"}]}}]}
    ...
    def test_cutoff_mismatch_is_explicitly_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            partial, market, matrix = self._inputs(Path(directory))
            value = json.loads(market.read_text())
            value["tickers"][0]["source_receipts"]["quote"] = {**value["tickers"][0]["source_receipts"]["quote"], "known_at": "2026-07-26T00:00:00Z"}
            market.write_text(json.dumps(value), encoding="utf-8")
            ...
            self.assertIn("market_fundamentals_quote_known_at_after_cutoff", result["models"][0]["blockers"])

    def test_earlier_source_captures_are_valid_at_declared_research_cutoff(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            partial, market, matrix = self._inputs(Path(directory))
            earlier = "2026-07-24T00:00:00Z"
            market_value = json.loads(market.read_text())
            for source in market_value["tickers"][0]["source_receipts"].values():
                source["known_at"] = earlier
            market.write_text(json.dumps(market_value), encoding="utf-8")
            partial_value = json.loads(partial.read_text())
            partial_value["companion_receipt_sha256"] = hashlib.sha256(market.read_bytes()).hexdigest()
            partial_value["models"][0]["model"]["as_of"] = earlier
            partial.write_text(json.dumps(partial_value), encoding="utf-8")
            result = compile_context_pack_models(partial, market, matrix, as_of=AS_OF)
            context = result["models"][0]["context_pack"]
            self.assertEqual(context["as_of"], AS_OF)
            self.assertEqual(context["official_context_as_of"], earlier)
            self.assertEqual(context["market_fundamentals"]["status"], "available")

    def test_matrix_research_cutoff_must_match_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            partial, market, matrix = self._inputs(Path(directory))
            value = json.loads(matrix.read_text())
            value["research_cutoff"] = "2026-07-25T01:00:00Z"
