...
MARKET_FACT = _spec(
    ...
    "高", True, ["交易所/官方行情", "次级行情源"],
    "按交易日抓取并附 as_of、交易所、币种与原始响应哈希；N1-3 已验证 A/HK/US/JP 历史价格主链，但不能据此推断历史估值。",
    "产品重建来源已验证；归档原始提供方仍不可伪装成已确认",
)
...
MAIN_SPECS: dict[str, dict[str, Any]] = {
    ...
    "mcap": clone(MARKET_FACT, "快照市值（归档口径）。", confidence="中", candidate_sources=["同日价格", "SEC companyfacts（美国）", "交易所/公司披露股本"], production_method="同日价格×披露日前已知股本；多类别股份、ADR 比率或 PIT 股本不全时留 gap。"),
    "mcap_usd": clone(MARKET_FACT, "快照美元市值（归档口径）。", confidence="中", candidate_sources=["同日本币市值", "Yahoo 历史 FX"], production_method="同日本币市值×同日冻结 USD 换算率；没有 PIT 市值或同日 FX 时留 gap。"),
    "pe": clone(MARKET_FACT, "市盈率快照。", confidence="中", candidate_sources=["同日市值", "SEC/company filing 的披露日前已知 TTM 净利", "提供方同日估值快照"], production_method="同日市值/TTM 净利；必须区分 GAAP、adjusted、亏损和 filing known_at。"),
    "pb": clone(MARKET_FACT, "市净率快照。", confidence="中", candidate_sources=["同日市值", "SEC/company filing 的披露日前已知净资产", "提供方同日估值快照"], production_method="同日市值/净资产；必须记录股东权益口径和 filing known_at。"),
    "peg": clone(DERIVED, "PEG 估值指标。", confidence="低", candidate_sources=["PE 快照", "版本化增长率快照"], fallback=["无法确认增长率定义时留空或标 definition_mismatch"], production_method="归档未披露增长率基础；SEC TTM 同比只能作为另一种定义，不得冒充 benchmark PEG。"),
...
LEVEL_SPECS: dict[str, dict[str, Any]] = {
    ...
    "mcap_yi": clone(MARKET_FACT, "等级页人民币亿元市值快照。", confidence="中", candidate_sources=["同日价格", "PIT 股本", "同日冻结 FX"], production_method="同日价格×PIT 股本并按同日 FX 换算；输入不全时留 gap。"),
    "mcap_usd_b": clone(MARKET_FACT, "等级页美元十亿美元市值快照。", confidence="中", candidate_sources=["同日本币市值", "Yahoo 历史 FX"], production_method="同日本币市值×同日冻结 USD 换算率；输入不全时留 gap。"),
    "pe": clone(MARKET_FACT, "等级页市盈率快照。", confidence="中", candidate_sources=["同日市值", "披露日前已知 TTM 净利"], production_method="同日市值/TTM 净利；GAAP 与 adjusted 口径不可混用。"),
    "peg": clone(DERIVED, "等级页 PEG。", confidence="低", candidate_sources=["PE 快照", "版本化增长率快照"], fallback=["定义不明时标 definition_mismatch"], production_method="归档增长率基础未披露；#114 只验证分档公式，不猜输入定义。"),
