...
def _load_checkpoint(path: Path, *, ticker: str) -> dict:
    if not path.exists():
        return {"ticker": ticker, "chapters": {}, "provider_receipts": []}
    try:
...
def main() -> int:
    ...
    parser.add_argument("ticker", choices=sorted(ISSUERS))
    ...
    narrative_path = (
        ...
        / "artifacts"
        / "evidence"
        / f"{args.ticker}-official-narrative-evidence.json"
    )
    financial_path = (
        ROOT
        / "artifacts"
        / "evidence"
        / f"{args.ticker}-financial-page-evidence.json"
    )
    ...
    lock_path = args.runtime_dir / f"{args.ticker}.lock"
    ...
    checkpoint_path = args.runtime_dir / f"{args.ticker}.checkpoint.json"
    checkpoint = _load_checkpoint(checkpoint_path, ticker=args.ticker)

    chapters: list[dict] = []
    provider_receipts: list[dict] = list(checkpoint.get("provider_receipts") or [])
    for spec in RESEARCH_SECTION_SPECS_V3[:-1]:
        evidence = list(evidence_by_section[spec.section_id])
        # Keep the conclusion input bounded to its own frozen evidence.  The
        # prior chapters are already persisted in the dossier and do not need
        # to be repeated in the model prompt (which caused provider truncation).
        prior = []
        if prior:
            used = {
                evidence_id
            ...
            evidence.extend(
                ...
                for evidence_id in sorted(used.difference(selected))
            )
            evidence_by_section[spec.section_id] = evidence
        base_request = build_chapter_request(
            spec=spec,
            issuer=ISSUERS[args.ticker],
            evidence=evidence,
        ...
        chapter.setdefault(
            ...
            (
                ...
                <= spec.target_characters[1]
                    ...
                    if chapter["character_count"] < spec.target_characters[0]
    ...
    rendered_html = render_html(
        markdown,
        title=f"{ISSUERS[args.ticker]['short_name']} Round 7 公司档案",
    )
    dossier["artifacts"] = {
        "markdown_path": str(args.output_dir / f"{args.ticker}.md"),
        "markdown_sha256": hashlib.sha256(markdown.encode()).hexdigest(),
        "html_path": str(args.output_dir / f"{args.ticker}.html"),
    ...
    _write(args.output_dir / f"{args.ticker}.md", markdown)
    _write(args.output_dir / f"{args.ticker}.html", rendered_html)
    _write(
        args.output_dir / f"{args.ticker}.receipt.json",
        json.dumps(dossier, ensure_ascii=False, indent=2) + "\n",
    )
    _write(
        args.output_dir / f"{args.ticker}.review-queue.json",
