...
402, while the 08:20 launchd environment had a minimal PATH and could not
resolve the `codex` executable. PR #1030 (`ce8011f`) now resolves the explicit
Homebrew/vendor binary for both Daily asset and thesis fallback and records
the executable in the receipt. A launchd-equivalent `PATH=/usr/bin:/bin`
probe returned structured JSON.

The subsequent real run used the same frozen source boundary: 31/31 data slots
ready, 18/19 asset explanations model-generated by Codex, one explicit ETH
deterministic readout, and a Codex-backed cross-asset thesis. The current
reader is `/Users/wendy/Desktop/K线日报/latest.md`, with the corresponding
HTML and article payload in the same directory. The 8907 Weekly historical
sample is now recorded as a QA control in
`docs/qa/market-regime-8907-control.md`; its prices and conclusions are never
copied into live editions.
...
Real local report proof: `report_id=market-regime-weekly-report:f4221605f3ad02e7ef5f314b08eab8b4885c5f5d33dd94a1189caaf859eea6fd`,
...
Focused Weekly suite: 138/138. Full repository baseline: 1122 tests ran with
...
strict: `cache_policy=bypass / quality=strict`; A-share indices additionally
declare `fallback_policy=explicit / fallback_sources=sina_index`, while all
...
bound to `build_sha=5cd7472036dec95e4eaf5e8f1e0b71b7b4c65eb0`, runtime root
...
without dropping malformed rows. No cached or undeclared source was used.
...
dirty worktree `/Users/wendy/datafeed` and database path were preserved; plist
snapshots are retained at
`/Users/wendy/Library/LaunchAgents/com.wendy.datafeed.plist.pre-cutover-20260820`
and
`/Users/wendy/Library/LaunchAgents/com.wendy.datafeed.plist.pre-issue21-20260820`.

Next: keep the ordered fallback chain visible in each response and add another
named source only through the same explicit contract. Do not add silent source
switching or downgrade strict `cache_policy=bypass / quality=strict` semantics.
...
`market-regime-weekly-report:38a44b287e29bfc05a1ee9efe049b4b4509ba378436bd1421adcc96041f39e3d`,
