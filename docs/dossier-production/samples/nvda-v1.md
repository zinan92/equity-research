...
| 假设 | 支持证据 | 可证伪条件 | 当前判断 |
| --- | --- | --- | --- |
| 加速计算平台拥有生态与系统级黏性 | 公司披露其平台涵盖 GPU、网络，并服务多类客户；FY2026 提出 NVLink Fusion。 [F-01] | 若大客户将训练/推理工作负载稳定迁移到替代平台、且不再需要其软硬件栈。 | **中等置信度的研究判断**：证据支持平台范围，不足以单独量化客户转换成本。 |
| Data Center 是增长引擎 | FY2026 Data Center 收入同比 +68%。 | 若后续报告显示 Data Center 增长显著低于总收入或出现持续客户集中度恶化。 | **事实支持强，持续性判断待更新**。 |
...
- **待补问题**：客户集中度、供应商依赖、回购/股权激励、区域收入与竞争对手替代，需要从 10-K 注释、proxy 和后续 10-Q 再建证据包。
...
| 采集/写作耗时 | 约 20 分钟（模板首样例的人工研究与写作；非自动化成本基线） |
| 模型/人工介入 | 人工选择官方 SEC、IR 与公司 Newsroom 来源；未使用第三方档案文本；当前 runner 未提供可核验 token 计数，后续自动化运行必须补该字段。 |
| 已用来源数 | 3 |
...
| S-02 | NVIDIA Newsroom | Jensen Huang bio | accessed 2026-07-23 | https://nvidianews.nvidia.com/bios/jensen-huang | 创立年份、CEO 角色、GPU 历史表述 |
